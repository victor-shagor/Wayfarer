  ...
  useEffect(() => {
    async function getData() {
      const get = await fetch(
        `/api/v1/bookings?origin=${originquery}&destination=${destinationquery}`,
        {
          method: "GET",
          headers: {
            "Content-type": "application/json",
  ...
  return (
    <div>
      ...
      <TabDiv>
        ...
        <Table style={{ color: "#00bfa6" }} color={"green"} className='booking-table'>
          <Table.Header>
            <Table.Row>
          ...
          <Table.Body>
            {trips.map((trip: any) => (
              <Table.Row key={trip.id} className='booking-table-rows'>
